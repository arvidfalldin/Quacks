...
from policy.utils import explosion_probability


class BrewingAction(Enum):
    STOP = 0
    DRAW = 1
    POTION = 2
...
class AlwaysExplodePolicy(BrewingPolicyBase):
    ...
    def __call__(self, observation):
        return BrewingAction.DRAW  # Draw a chip no matter what
...
class StopAtProbabilityPolicy(BrewingPolicyBase):
    ...
    def __call__(self, observation):
        ...
        return BrewingAction.DRAW if prob <= self.threshold else BrewingAction.STOP
